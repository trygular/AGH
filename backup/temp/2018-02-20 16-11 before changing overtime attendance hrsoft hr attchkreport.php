...
if($_POST['action']=="Generate") 
{
	...
	echo '<form action="attchkreportexcel.php" method="POST"><br><span style="float:right;"><input class="btn btn-primary" style="font-weight:bold" type="submit" name="Action" id="Action" value="Generate as Excel"> <input class="btn btn-primary" style="font-weight:bold" type="submit" name="Action" id="Action" value="Generate as PDF"></span>
	';
...
//------------------------- PERFORMANCE
	if($_POST['misreport']=='performance') 
	{
		...
		while($row1 = fetch($StaffMasterRes))
		{
			...
			echo '<tr><td colspan="32"><center><span class="btn btn-warning" style="font-weight:bold;fontsize:15px;"> Empcode : '.$row1[0].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Name : '.$row1[1].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Department : '.$DepartmentMatserRow[1].'&nbsp;&nbsp;Shift : '.$row1['shiftid'].' </center></span></td></tr><br>';
			   ...
			   //echo $lastday ;
					...
					*/
						...
						*/
							...
							if($latemin < 59)
								echo"<td style='color:black;font-size:12px;font-face:verdana;'>".$latemin." min</td>";
							else if($latemin > 59)
								echo"<td style='color:black;font-size:12px;font-face:verdana;'>".$latehrs.'h '.$submi."min</td>";
								
								/*echo"<td style='color:black;font-size:12px;font-face:verdana;'>Y (".($latehrs / 60) .")</td>";*/
							//else
							//	echo"<td style='color:black;font-size:12px;font-face:verdana;'>Y (".$latemin.")</td>";
						}
					...
					$shifttotaltime = $ShiftMasterRow[8];
						...
						if( strtotime($rowattendance[5]) < strtotime('17:30:00') && $rowattendance[5] != "" )
						{
							...
							/*
								...
								echo"<td style='color:black;font-size:12px;font-face:verdana;'>".$latehrs.':'.$submi."</td>";
							*/
			...
			for($i=1;$i<=$lastday;$i++) 
			{
				...
				if($rowattendance!=NULL)
				{
					echo"<td style='color:black;font-size:12px;font-face:verdana;'>".$rowattendance[15]."".$rowattendance[15]."</td>";
				}
				else if($rowday[0]==$weekdayRow[1])
				{	
				   echo"<td style='color:blue;font-size:12px;font-face:verdana;'>WO</td>";
		...
		//echo "EarlyDept smas= ".$StaffMasterSql;
		   ...
		   while($StaffMasterRow = fetch($StaffMasterRes))
		   {
				...
				while($rowlatearrival = fetch($reslatearrival))
				{
					...
					//echo "<br />1 -".$rowlatearrival[6];
					if($rowlatearrival[6]!=null) 
					{		
						//echo "<br />2-".(strtotime($rowlatearrival[6]) < strtotime($rowshifttime[7]));
						if(strtotime($rowlatearrival[6])< strtotime($rowshifttime[7]))
						{
							...
							$restimediff = exequery($qrytimediff);
							$rowtimediff = fetch($restimediff);
							//echo "<br />3-".$qrytimediff;
							
							$qrytimediff1 = "SELECT TIMEDIFF('$daytemp $rowtimediff[0]','$daytemp 00:11:00')";
							$restimediff1 = exequery($qrytimediff1);
							$rowtimediff1 = fetch($restimediff1);
							$tempdata = substr($rowtimediff1[0],0,1);
							//echo "<br />4-".$qrytimediff1;
	...
	$StaffMasterSql = "select * from StaffMaster where lefts!=1 ";
	 ...
	 if($_POST['company']!='all')
		...
		$StaffMasterSql.=" Order BY region,deptid,Empcompanyid";
			...
			//--
		//echo "overtime smast= ".$StaffMasterSql;
		   $StaffMasterRes = exequery($StaffMasterSql);
		   while($StaffMasterRow = fetch($StaffMasterRes))
		   {
				$region="select * from RegionMaster1 where regionid='".$StaffMasterRow['region']."'";
				...
				//echo "<br>qrylatearrival=".$qrylatearrival;
				while($rowlatearrival = fetch($reslatearrival))
				{
					$ShiftMasterSql = "select * from ShiftMaster where shiftid ='".$StaffMasterRow['shiftid']."'";
					$ShiftMasterRes = exequery($ShiftMasterSql);
					...
					//echo "<br>5- ".$qrydifference."  str = ".$rowdifference[0]."  substr=".substr($rowdifference[0],1,1);
...
<script type="text/javascript">
	...
	<table class="table">
		<tr>
		<td><input  type="radio"  id="performance" name="misreport" value="performance" checked>Performance</td>
		<td><input  type="radio"  id="absent" name="misreport" value="absent" >Absent</td>
		<td><input  type="radio"  id="ab" name="misreport" value="ab" >No Punch</td>
		<td><input  type="radio"  id="latearrival" name="misreport" value="latearrival" >Late Arrival</td>
		<td><input  type="radio"  id="earlydep" name="misreport" value="earlydep" >Early Departure</td>
		<td><input  type="radio"  id="overtime" name="misreport" value="overtime" >Over Time</td>
		<td><input  type="radio"  id="od" name="misreport" value="od">OD</td>
		<td><input  type="radio"  id="status" name="misreport" value="status" >Status</td>
...
<tr id='display2' style="visible:hidden;">
	<td>First In <input  type="checkbox"  id="firstin" name="firstin" value="firstin" checked></td>
	<td>First Out <input  type="checkbox"  id="firstout" name="firstout" value="firstout" ></td>
	<td>Second In <input  type="checkbox"  id="secondin" name="secondin" value="secondin" ></td>
	<td>Second Out <input  type="checkbox"  id="secondout" name="secondout" value="secondout"></td>
</tr>
