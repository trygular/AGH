...
function getEmpTable($empcodes, $myindex, $myid)
{
	$data = "";
	$data .= "<table border='1' id='displaytbl".$myindex.$myid."' class='silent'>";
	$data .= "<tr><th>ID</th><th>Name</th><th>Region</th><th>Branch</th></tr>";
	for($i=0; $i<sizeof($empcodes)-1; $i++)
	{	
		$qinrb = "SELECT S.empcode, S.Name, R.regionname, B.branchname FROM `Lokmanya-Empire`.StaffMaster S, `Lokmanya-Empire`.BranchMaster B, `Lokmanya-Empire`.RegionMaster R WHERE B.branchcode = S.Branchid AND R.regioncode = B.regioncode AND empcode = '".$empcodes[$i]."'";
		$resinrb = exequery($qinrb);
		$data .= "<tr><td>".$empcodes[$i]."</td><td>".$resinrb[1]."</td><td>".$resinrb[2]."</td><td>".$resinrb[3]."</td></tr>";
	}
	/*while($rowinrb = fetch($resinrb))
	{
			...
			while($rowqry=fetch($resqry))
			{
				...
				while($rowquest=fetch($resqryques))
				{
					if($rowqry[0]!=9 && $rowqry[0]!=10)
					{
						...
						echo"<td style='text-align:center;width:100px' onclick='tabledata(".$tabindex.",1);'>".$cntone."</td>
							<td style='text-align:center;width:100px' onclick='tabledata(".$tabindex.",2);'>".$cnttwo."</td>
							<td style='text-align:center;width:100px' onclick='tabledata(".$tabindex.",3);'>".$cntthr."</td>";
						...
						echo getEmpTable($arrone[$i], $tabindex, "one");
						echo getEmpTable($arrtwo[$i], $tabindex, "two");
						echo getEmpTable($arrthr[$i], $tabindex, "thr");
...
<form method="POST" Action="GraphReport2.php" name="form">
	<center>
	<br>
