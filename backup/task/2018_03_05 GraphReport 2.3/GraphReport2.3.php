...
<?
  ...
  {
?>	  
<input class='btn btn-danger' type='button' id='back' name='back' value='Back' style='float:right;margin-right:20px' onclick="window.location='GraphReport2.3.php'">
...
<?php
		...
		if($_POST["question"] != 'ALL' || $_POST["subquestion"] != 'ALL')
		{
			...
			//echo "alert(".$validnot.");";
			//echo "alert(".$validall.");";
			//echo "alert(".$valid1."=".$valid2."=".$valid3."=".$validnot."=".$validall.");";
		} else { //get total's
...
?>
		...
		<?
			...
			while($rowqry=fetch($resqry))
			{
				...
				echo"<tr class='propty_tab_".++$cnttrtogg."' onclick='getdetails(".$cnttrtogg.");'> <th colspan=7 style='text-align:left;background-color:#f1b25b;cursor:pointer;margin-right:250px'>$rowqry[1]</th> </tr>";
				...
				//create 5 hidden inputs > total	id=tot_tab$cnttrtogg
				echo "<input type='hidden' id='tot_01_tab".$cnttrtogg."' value='".json_encode($json_tot01)."' />";
				echo "<input type='hidden' id='tot_02_tab".$cnttrtogg."' value='".json_encode($json_tot02)."' />";
				echo "<input type='hidden' id='tot_03_tab".$cnttrtogg."' value='".json_encode($json_tot03)."' />";
				echo "<input type='hidden' id='tot_not_tab".$cnttrtogg."' value='".json_encode($json_totnot)."' />";
				echo "<input type='hidden' id='tot_all_tab".$cnttrtogg."' value='".json_encode($json_totall)."' />";
				
				
			}
			//create 5 hidden inputs > GrandTotal	id=tot_tab$cnttrtogg
...
if( $_POST["question"] == "ALL" && $_POST["subquestion"] == "ALL")
{
	...
	//echo "alert(".$cnttrtogg.");";
	
	for($i=0; $i<=$cnttrtogg; $i++)
	{
		if($_POST["question"] != $i)	// hide all TR > class = propty$i and propty_tab_$i
			echo "$('.propty".$i."').hide();"."$('.propty_tab_".$i."').hide();";
