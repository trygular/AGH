...
<?
  ...
  {
?>	  
<input class='btn btn-danger' type='button' id='back' name='back' value='Back' style='float:right;margin-right:20px' onclick="window.location='GraphReport2.1.php'">
		 ...
		 while($rowquest = fetch($resquest))
		 {
			...
			while($rowqry=fetch($resqry))
			{
				if($rowqry[1]==0&&$rowqry[2]==0&&$rowqry[3]==0)	
				{
					...
					window.location="GraphReport2.1.php";
					</script>
					<?
				}	
			}
...
?>		 
		...
		<center><div style='text-align:right;'><input type='button' id='hideall' name='hideall' value='Close All' onclick="$('table .silent').hide();" style='margin-right: 20px;' /></div>
			...
			while($rowqry=fetch($resqry))
			{
				...
				while($rowquest=fetch($resqryques))
				{
					if($rowqry[0]!=9 && $rowqry[0]!=10)
					{
						...
						echo"<tr class='propty".$cnttrtogg++."'>
						...
						echo "</tr><tr><td colspan='2'></td><td colspan='6'>";
...
<div class="dialog" id="myform" style="width:800px;">
	<div id="panel" style="text-align:left;"> <input type="button" class="" value="Close" id="btnOK" /> </div>
<? 
...
<form method="POST" Action="GraphReport2.1.php" name="form">
	<center>
	<br>
