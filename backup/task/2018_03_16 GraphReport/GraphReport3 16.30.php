...
?>
	...
	<!-- Back button -->
	<button class='btn' id='back' name='back' style='float:right;' onclick="window.location='GraphReport3.php'">BACK</button>
...
<?php
	...
	if( $_POST["question"] == "ALL" && $_POST["subquestion"] == "ALL")
	{
		...
		$graph_header = "<b>All Regions</b>";
		echo "var chartHeader = '';";
...
?>
	...
	for(cnt = 1; cnt <= json_tot_not.length; cnt++)
	{
		if(json_tot_not.length > 5) {
			var cntnext = Math.floor((json_tot_not.length/2) + cnt);
			
			printData1 = ""+cnt + ". " + $("[name='subq" + cnt + "']").text();
			printData2 = ""+cntnext + ". " + $("[name='subq" + cntnext + "']").text();
			
			if(cnt <= (json_tot_not.length/2)+1 )
				printSubQ += "<tr><td style='width:50%;' class='chartQuestion'>" + printData1 + "</td><td style='width:50%;' class='chartQuestion'>" + printData2 + "</td></tr>";
		} else {
			printData = cnt + ". " + $("[name='subq" + cnt + "']").text();
...
<?php
	...
	echo "var chartHeader = '';";
	echo "chartHeader += $('#totTab".$_POST["question"]."').text(); ";
	if($_POST['empcode']!='')
	{
...
<?
	...
	while($rowqry = fetch($resqry))
	{
		...
		na = not attempted
		*/
		echo "<tr class='propty_tab_".++$cnt_qid."' onclick='toggleTabs(".$cnt_qid.");'>
		<th id='totTab".$cnt_qid."' colspan=2 style='text-align:left;background-color:#f1b25b;cursor:pointer;margin-right:250px'>$rowqry[1]</th>
		<th id='totTab".$cnt_qid."_sd' style='background-color:#f1b25b;cursor:pointer;margin-right:250px'></th>
		<th id='totTab".$cnt_qid."_no' style='background-color:#f1b25b;cursor:pointer;margin-right:250px'></th>
		<th id='totTab".$cnt_qid."_sa' style='background-color:#f1b25b;cursor:pointer;margin-right:250px'></th>
		<th id='totTab".$cnt_qid."_na' style='background-color:#f1b25b;cursor:pointer;margin-right:250px'></th>
		<th id='totTab".$cnt_qid."_to' style='background-color:#f1b25b;cursor:pointer;margin-right:250px'></th>
		</tr>";
		...
		{	
			if($rowqry[0]!=9 && $rowqry[0]!=10)
			{
				...
				echo "<td style='width:1000px' name='subq".$rowquest[0]."' id='question_".$cnt_qid."_".$tabindex."' class='subquestion_".$cnt_qid."_".$cntzro++."'>$rowquest[1]</td>";
				
				
				$onclick0 = " onclick='alert(\"Count is Zero...!\");'";
				$onclick1 = " onclick='dialogDisplay(".$tabindex.",1,\"".$rowquest[1]."\");'";
				$onclick2 = " onclick='dialogDisplay(".$tabindex.",2,\"".$rowquest[1]."\");'";
				$onclick3 = " onclick='dialogDisplay(".$tabindex.",3,\"".$rowquest[1]."\");'";
		...
		echo "<script> $('#totTab".$cnt_qid."_sd').text('".array_sum($json_tot01)."'); </script>";
		echo "<script> $('#totTab".$cnt_qid."_no').text('".array_sum($json_tot02)."'); </script>";
		echo "<script> $('#totTab".$cnt_qid."_sa').text('".array_sum($json_tot03)."'); </script>";
		echo "<script> $('#totTab".$cnt_qid."_na').text('".array_sum($json_totnot)."'); </script>";
		echo "<script> $('#totTab".$cnt_qid."_to').text('".array_sum($json_totall)."'); </script>";
		echo "</tr> ";
...
<form method="POST" Action="GraphReport3.php" name="form">
	<center>
	<br>
