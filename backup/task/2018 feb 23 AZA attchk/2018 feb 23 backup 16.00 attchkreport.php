	...
	while($row1 = fetch($StaffMasterRes))
	{
		...
		echo '</tr><tr><td>Arrival</td>';
			...
			for($i=1;$i<=$lastday;$i++) 
			{
				...
				else if($rowday[0] == $weekdayRow[1] || $rowday[0] == 'SATURDAY')
				{
...
if($_POST['misreport']=='status') 
{
	...
	echo "<center> <p class='btn btn-warning' style='font-color:white;font-weight:bold;font-size:16px;'>Region : ".$branch." &nbsp;&nbsp;&nbsp; Department :  ".$department." &nbsp;&nbsp;&nbsp; Company : ".$company." </p></center>";
	...
	while($StaffMasterRow = fetch($StaffMasterRes))
	{
		...
		echo '<center><table class="table table-bordered" border="1" style="width:80%">';

		echo '<tr>';
		echo "<td colspan='30'>Early Departure List  between ".$_POST['frmdate']." to ".$_POST['todate']." Employee Code :- <b>".$StaffMasterRow[0]."</b> &nbsp;&nbsp;&nbsp;&nbsp; Employee Name :- <b>".$StaffMasterRow[1]."</b></td>";
		echo '</tr>';
		...
		if($_POST['firstin']=='firstin' && $_POST['firstout']=='firstout') 
	 	{
			$wharrival = 0;
			$whdeparture = 0;
		    ...
		    for($i=0;$i<$days1;$i++) 
			{
				...
				if($rowattendance[4] != NULL)
				{
					$wharrival = $rowattendance[4];
				
					if($rowattendance[13]!=NULL) {
						$whdeparture = $rowattendance[13];
					}
					else if( $rowattendance[12]!=NULL )
					{
						$whdeparture = $rowattendance[12];
					}else if( $rowattendance[11]!=NULL )
					{
						$whdeparture = $rowattendance[11];
					}else if( $rowattendance[10]!=NULL )
					{
						$whdeparture = $rowattendance[10];
					}else if( $rowattendance[9]!=NULL )
					{
						$whdeparture = $rowattendance[9];
					}else if( $rowattendance[8]!=NULL )
					{
						$whdeparture = $rowattendance[8];
					}else if( $rowattendance[7]!=NULL )
					{
						$whdeparture = $rowattendance[7];
					} else if($rowattendance[6]!=NULL )
					{
						$whdeparture = $rowattendance[6];
					} else if($rowattendance[5]!=NULL )
					{
						$whdeparture = $rowattendance[5];
					}
					else 
					{
						$whdeparture = 0;
					...
					$whtimeSql = "select TIMEDIFF ('".$whdeparture."','".$wharrival."')";
					$whtimeRes = exequery($whtimeSql);
					$whtimeRow = fetch($whtimeRes);

					
					$tempovertime = substr($whtimeRow[0],0,1);
					if($tempovertime!='-'){
						 $worktime = substr($whtimeRow[0],0,5);
					} else{
						$worktime = "00:00";
					}
