...
<div id="main_container" class="container"> 
	...
	<hr />
		...
		<div class="row">
			...
			<div class="col-sm-3"><button type="button" id="btnsearch" class="btn btn-danger">DELETE STUDENT</button></div>
		</div>
		...
		<div class="row">
			...
			<div class="col-sm-3">NAME:<input type="text" class="form-control frm_control" id="sname" name="sname" placeholder="Enter name" /></div>
			<div class="col-sm-3"></div>
		</div>
		...
		<hr />
	<button type="button" id="form_add" class="btn btn-info" style="width:10%;">ADD</button>&nbsp;<button type="button" id="form_reset" class="btn btn-info" style="width:10%;">RESET</button> 
	</div>
...
<hr />
	<table class="table table-striped table-dark table-bordered" id="tab_add">
	<tr class="thead-dark"><th>MID</th><th>SID</th><th>Name</th><th>Marks1</th><th>Marks2</th><th>Marks3</th><th>Average</th><th>Edit</th><th>Delete</th></tr>
 ...
 document.getElementById("row"+no+"").outerHTML="";
	...
	$('#btnsearch').click(function(){
		$.ajax({
		  method: "POST",
		  url: "./studc.php",
		  data: { token: "deletestd", si:$("#sid").val() }
		})
		.done(function(msg) {
			alert(msg);
			$("#form_reset").click();
	...
	{
		if( $('sname').val() != '' && $('#tab_add tr').length > 1 ) 
		{
			$.ajax({
			  method: "POST",
			  url: "./studc.php",
			  data: { token: "insert", sname: $("#sname").val(), tabdata: jtc() }
			}).done(function(msg) {
				alert( msg );
				setTimeout(location.reload.bind(location), 2000);
			});
		} else { alert("Table data cannot be blank!"); }
	});
	...
	$("#form_update").click(function()
	{
		if( $('sname').val() != '' && $('#tab_add tr').length > 1 ) 
		{
			$.ajax({
			  method: "POST",
			  url: "./studc.php",
			  data: { token: "update", si:$("#sid").val(), sname: $("#sname").val(), tabdata: jtc() }
			}).done(function(msg) {
				alert( msg );
				setTimeout(location.reload.bind(location), 2000);
			});
	...
	$("#form_add").click(function(){
		...
		if($("#sname").val()=='') { $("#sname").val("").focus(); alert("Enter name!"); return;
		}else if($("#mid").val()==''){ $("#mid").val("").focus(); alert("Enter mid!"); return;
		}else if($("#sid").val()==''){ $("#sid").val("").focus(); alert("Enter sid!"); return;
		}else if($("#m1").val()==''){ $("#m1").val("").focus(); alert("Enter marks 1!"); return;
		}else if($("#m2").val()==''){ $("#m2").val("").focus(); alert("Enter marks 2!"); return;
		}else if($("#m3").val()==''){ $("#m3").val("").focus(); alert("Enter marks 3!"); return;
		}
	...
	}).focus(function(){ $(this).val(''); }).blur(function(){ avg(); });
